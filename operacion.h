...
class Operacion{
    ...
    bool parseExecute(const string& linea){
        if(linea.empty() || linea[0] == '#')return true; // si está limpia o tinee comentariors
        auto tokens = hacerToken(linea);
        if(tokens.empty()) return true;
        const string& operacion = tokens[0];
        if(operacion == "MEMORIA_TOTAL" || operacion == "TAMANO_PAGINA" || operacion == "TAMANO_CACHE_GLOBARL"){
            return true;
        }

        if(operacion == "NEW_PROCESS"){
            if(tokens.size() < 3){
                cout << "! Error, para hacer un NEW_PROCESS debe excribirse \"NEW_PROCESS <id> <memoria>\"" << endl;
                return false;
            }
        ...
        if(operacion == "ACCESS"){
            if(tokens.size() < 3){
                cout << "! Error, para hacer un ACCESS debe escribirse \"ACCESS <id> <direcccion>\"" << endl;
                return false;
            }
        ...
        if(operacion == "ALLOCATE"){
            if(tokens.size() < 3){
                cout << "! Error, para hacer un ALLOCATE debe escribirse \"ALLOCATE <id> <memoria>\"" << endl;
                return false;
            }
        ...
        if(operacion == "FREE"){
            if(tokens.size() < 4){
                cout << "! Error, para hacer un FREE debe escribirse \"FREE <id> <dirección> <bytes>\"" << endl;
                return false;
            }
        ...
        if(operacion == "PRINT_PROCESS"){
            if(tokens.size() < 2){
                cout << "! Error, para hacer un PRINT_PROCESS debe escribirse \"PRINT_PROCESS <id>\"" << endl;
                return false;
            }
    ...
    bool leerConfig(istream& stream, int& memTotal, int& tamPag, int& cacheGlobal, vector<string>& lineasRestantes){
        ...
        while(getline(stream, linea)){
            if(linea.empty() || linea[0] == '#'){
                continue; // si está en blanco o es un comentario se ignora
            }
            auto tokens = hacerToken(linea);
            if(tokens.empty()){
                continue;
    ...
    bool runFile(const string& nombre){
        ...
        void runInteractiveMode(){
            ...
            cout << " - END_PROCESS <id>" << endl;
            cout << " - ACCESS <id> <dirección>" << endl;
            cout << " - ALLOCATE <id> <memoria>" << endl;
            cout << " - FREE <id> <dirección inicla> <bytes>" << endl;
